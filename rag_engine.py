...
rag_engine.py
─────────────
Handles all wiki-rag and LLM (Anthropic / OpenAI) interactions.
...
def call_openai(
    ...
    try:
        ...
        raise RuntimeError("OpenAI rejected the API key. Check that it is valid.")
...
def parse_report_json(raw: str, query: str, sources: list[WikiSource]) -> ResearchReport:
    """Parse the LLM's JSON output into a ResearchReport, with a safe fallback."""
    cleaned = (
        raw.strip()
        .removeprefix("```json")
        .removeprefix("```")
...
def generate_report(
    ...
    `provider` selects which LLM backend writes the report: "anthropic"
    (default, requires anthropic_api_key) or "openai" (requires openai_api_key).
    """
    ...
    if provider == "openai":
        raw = call_openai(
            prompt, system=SYSTEM_PROMPT, api_key=openai_api_key, model=openai_model
        )
    else:
        raw = call_anthropic(
